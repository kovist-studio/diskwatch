...
| Updates | **None in-app** | `electron-updater` was planned and is not installed — it is not a dependency, and `electron-builder.yml` sets `publish: null` so the builder never publishes anything itself. Releases are cut by `.github/workflows/build.yml`, which builds with `--publish never` and then uploads artifacts with `gh release create`. Users update by installing a newer build; the channels in §9 (Homebrew Cask, winget, Scoop) are how that becomes one command. An in-app updater is a silent self-modifying write, which is a strange thing for this app to ship before it is actually needed. |
